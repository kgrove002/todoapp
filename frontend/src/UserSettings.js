...
function UserSettings({ refreshData, id, email, password, pin, login, firstName }) {
  ...
  const checkEmail = async (email) => {
    try {
      const response = await axios.post('http://73.216.67.101:5000/checkEmail', { email });
  ...
  const updateEmailDB = async (email) => {
    try {
      const response = await axios.post('http://73.216.67.101:5000/updateEmail', {
        id,
        email,
      });
  ...
  const updatePinDB = async (pin) => {
    try {
      const response = await axios.post('http://73.216.67.101:5000/updatePin', {
        id,
        pin,
      });
      return response.data; // Return the response data to the caller
  ...
  const updatePasswordDB = async (password) => {
    try {
      const response = await axios.post('http://73.216.67.101:5000/updatePassword', {
        id,
        password,
      });
      return response.data; // Return the response data to the caller
